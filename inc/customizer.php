<?php
/**
 * Amoeba Customizer functionality
 *
 * @package Amoeba
 * @since Amoeba 1.0
 */
 ...
 */
function amoeba_customize_register($wp_customize) {

	// Adds section in wordpress customizer.
	$wp_customize->add_section('common_settings', array(
		'title' => __('Common Settings')
	));

	$wp_customize->add_setting('google_analytics_tracking_id', array(
		'default' => '',
		'transport' => 'postMessage',
	));
	$wp_customize->add_control('google_analytics_tracking_id', array(
		'label'		=> __('Google Analytics Tracking ID', 'amoeba'),
		'section'	=> 'common_settings',
		'type'		=> 'text'
	));
}
add_action('customize_register', 'amoeba_customize_register');

/**
 * Render the site title for the selective refresh partial.
 *
 * @since Amoeba 1.5
 * @see amoeba_customize_register()
 *
 * @return void
 */
function amoeba_customize_partial_blogname() {
	bloginfo('name');
}
 ...
 * @return void
 */
function amoeba_customize_partial_blogdescription() {
	bloginfo('description');
}

/**
 * Register color schemes for Amoeba.
 *
 * Can be filtered with {@see 'amoeba_color_schemes'}.
 *
 ...
 */
function amoeba_get_color_schemes() {
	/**
	 * Filter the color schemes registered for use with Amoeba.
	 *
	 ...
	 * }
	 */
	return apply_filters('amoeba_color_schemes', array(
		'default' => array(
			'label'  => __('Default', 'amoeba'),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if (! function_exists('amoeba_get_color_scheme')) :
/**
 * Get the current Amoeba color scheme.
 *
 * @since Amoeba 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function amoeba_get_color_scheme() {
	$color_scheme_option = get_theme_mod('color_scheme', 'default');
	$color_schemes       = amoeba_get_color_schemes();
...
endif; // amoeba_get_color_scheme

if (! function_exists('amoeba_get_color_scheme_choices')) :
/**
 * Returns an array of color scheme choices registered for Amoeba.
 *
 * @since Amoeba 1.0
 *
 * @return array Array of color schemes.
 */
function amoeba_get_color_scheme_choices() {
	$color_schemes                = amoeba_get_color_schemes();
...
endif; // amoeba_get_color_scheme_choices

if (! function_exists('amoeba_sanitize_color_scheme')) :
/**
 ...
 */
function amoeba_sanitize_color_scheme($value) {
	$color_schemes = amoeba_get_color_scheme_choices();
...
endif; // amoeba_sanitize_color_scheme
 ...
 */
function amoeba_color_scheme_css() {
	$color_scheme_option = get_theme_mod('color_scheme', 'default');
	...
	$color_scheme = amoeba_get_color_scheme();

	// Convert main and sidebar text hex color to rgba.
	$color_textcolor_rgb         = amoeba_hex2rgb($color_scheme[3]);
	$color_sidebar_textcolor_rgb = amoeba_hex2rgb($color_scheme[4]);
	$colors = array(
	...
	$color_scheme_css = amoeba_get_color_scheme_css($colors);

	wp_add_inline_style('amoeba-style', $color_scheme_css);
}
add_action('wp_enqueue_scripts', 'amoeba_color_scheme_css');
 ...
 * @since Amoeba 1.0
 */
function amoeba_customize_control_js() {
	...
	wp_localize_script('color-scheme-control', 'colorScheme', amoeba_get_color_schemes());
}
add_action('customize_controls_enqueue_scripts', 'amoeba_customize_control_js');
 ...
 * @since Amoeba 1.0
 */
function amoeba_customize_preview_js() {
	wp_enqueue_script('amoeba-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array('customize-preview'), '20141216', true);
}
add_action('customize_preview_init', 'amoeba_customize_preview_js');
 ...
 */
function amoeba_get_color_scheme_css($colors) {
 ...
 * @since Amoeba 1.0
 */
function amoeba_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-amoeba-color-scheme">
		<?php echo amoeba_get_color_scheme_css($colors); ?>
	</script>
	<?php
}
add_action('customize_controls_print_footer_scripts', 'amoeba_color_scheme_css_template');
